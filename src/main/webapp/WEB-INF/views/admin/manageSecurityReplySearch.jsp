	...
	<body>
        ...
        <main>
            ...
            <div id="main_middle">
                ...
                <div class="search">
                	<select id="boardType">
                		<option value="hobby">취미 댓글</option>
                		<option value="security" selected>안전 댓글</option>
                	</select>
                	<br>
	                    ...
	                    <tbody class="table-group-divider">
							<c:forEach var="reply" items="${sList}">
								<tr>
									...
									<td>${reply.sCommentContent }</td>
									<td style="text-align:center">${reply.userNo }</td>
									<td style="text-align:center">
										<button class="userDelete" onclick="if (confirm('삭제 하시겠습니까?')) { location.href = '/comment/commentdelete.do?sCommentNo=${reply.sCommentNo }'; window.location.href = window.location.href; }">삭제</button>
									</td>
								</tr>
							</c:forEach>
						</tbody>
						<tfoot>
							<tr align="center">
								...
								<c:if test="${pInfo.currentPage != 1 }">
									<a href="/manageReply/search.do?selectedValue=security&currentPage=${pInfo.currentPage - 1 }&
